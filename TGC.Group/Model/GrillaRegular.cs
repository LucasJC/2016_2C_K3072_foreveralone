...
namespace TGC.Examples.Optimization.GrillaRegular
{
    ...
    public class GrillaRegular
    {
        ...
        private List<InteractiveObject> modelos;
        ...
        public void create(List<InteractiveObject> modelos, TgcBoundingAxisAlignBox sceneBounds)
        {
            ...
            foreach (InteractiveObject modelo in modelos)
            {
        ...
        ///     Construye la grilla
        /// </summary>
        private GrillaRegularNode[,,] buildGrid(List<InteractiveObject> modelos, TgcBoundingAxisAlignBox sceneBounds, Vector3 cellDim)
        {
        ...
        /// </summary>
        private void addModelsToCell(GrillaRegularNode node, List<InteractiveObject> modelos)
        {
            foreach (InteractiveObject modelo in modelos)
            {
                if (TgcCollisionUtils.testAABBAABB(node.BoundingBox, modelo.mesh.BoundingBox))
                {
                    node.Models.Add(modelo.mesh);
                }
            }
        }
