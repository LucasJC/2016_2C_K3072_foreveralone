...
namespace TGC.Group.Model.Optimization
{
    ...
    internal class QuadtreeBuilder
    {
        ...
        public QuadtreeNode crearQuadtree(List<InteractiveObject> models, TgcBoundingAxisAlignBox sceneBounds)
        {
            ...
            doSectorQuadtreeX(rootNode, center, midSize, 0, models);
        ...
        private void doSectorQuadtreeX(QuadtreeNode parent, Vector3 center, Vector3 size,
            int step, List<InteractiveObject> models)
        {
            var x = center.X;

            //Crear listas para realizar corte
            var possitiveList = new List<InteractiveObject>();
            var negativeList = new List<InteractiveObject>();

            //X-cut
            var xCutPlane = new Plane(1, 0, 0, -x);
            splitByPlane(xCutPlane, models, possitiveList, negativeList);
        ...
        private void doSectorQuadtreeZ(QuadtreeNode parent, Vector3 center, Vector3 size, int step,
            List<InteractiveObject> meshes, int childIndex)
        {
            var z = center.Z;
        ...
        /// </summary>
        private void splitByPlane(Plane cutPlane, List<InteractiveObject> modelos,
            List<InteractiveObject> possitiveList, List<InteractiveObject> negativeList)
        {
            TgcCollisionUtils.PlaneBoxResult c;
            foreach (var modelo in modelos)
            {
                c = TgcCollisionUtils.classifyPlaneAABB(cutPlane, modelo.mesh.BoundingBox);
        ...
        private void optimizeSectorQuadtree(QuadtreeNode[] children)
        {
            ...
            for (var i = 0; i < children.Length; i++)
            {
                ...
                if (childNodeChildren != null && hasEmptyChilds(childNode))
                {
                    ...
                    childNode.models = new InteractiveObject[0];
                }
                else
                {
